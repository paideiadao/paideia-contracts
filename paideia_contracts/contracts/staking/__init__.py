...
class InvalidInputBoxException(Exception):
    pass


class InvalidTransactionConditionsException(Exception):
    pass
...
class PaideiaStakingContract(ErgoContractBase):
    def __init__(
        ...
        stakingConfig,
        script: str = None,
        mapping: Dict[str, ErgoValue] = {},
        ergoTree: ErgoTree = None,
    ) -> None:
    ...
    def __init__(self, stakingConfig) -> None:
        mapping = {
            "_stakeStateNFT": ErgoAppKit.ergoValue(
                stakingConfig.stakeStateNFT, ErgoValueT.ByteArrayFromHex
            ).getValue(),
            "_emissionNFT": ErgoAppKit.ergoValue(
                stakingConfig.emissionNFT, ErgoValueT.ByteArrayFromHex
            ).getValue(),
        }
        super().__init__(
            stakingConfig,
            script=os.path.join(
                ...
                f"ergoscript/{stakingConfig.version}/stake.es",
            ),
            mapping=mapping,
        )
...
class StakeStateContract(PaideiaStakingContract):
    def __init__(self, stakingConfig) -> None:
        mapping = {
            "_stakedTokenID": ErgoAppKit.ergoValue(
                stakingConfig.stakedTokenId, ErgoValueT.ByteArrayFromHex
            ).getValue(),
            "_stakePoolNFT": ErgoAppKit.ergoValue(
                stakingConfig.stakePoolNFT, ErgoValueT.ByteArrayFromHex
            ).getValue(),
            "_emissionNFT": ErgoAppKit.ergoValue(
                stakingConfig.emissionNFT, ErgoValueT.ByteArrayFromHex
            ).getValue(),
            "_stakeContractHash": ErgoAppKit.ergoValue(
                blake2b(
                    bytes.fromhex(stakingConfig.stakeContract._ergoTree.bytesHex()),
                    digest_size=32,
                ).digest(),
                ErgoValueT.ByteArray,
            ).getValue(),
        }
        super().__init__(
            stakingConfig,
            script=os.path.join(
                os.path.dirname(__file__),
                f"ergoscript/{stakingConfig.version}/stakeState.es",
            ),
            mapping=mapping,
        )
...
class StakePoolContract(PaideiaStakingContract):
    def __init__(self, stakingConfig) -> None:
        mapping = {
            ...
            "_emissionFeeAddress": ErgoAppKit.ergoValue(
                "0008cd02189359b825e96aa3c7af90c9958d85daf8f86358382db3306e024c5aeea1e8ec",
                ErgoValueT.ByteArrayFromHex,
            ).getValue(),
        }
        super().__init__(
            stakingConfig,
            script=os.path.join(
                os.path.dirname(__file__),
                f"ergoscript/{stakingConfig.version}/stakePool.es",
            ),
            mapping=mapping,
        )
    ...
    def __init__(self, stakingConfig) -> None:
        mapping = {
            ...
            "_stakeTokenID": ErgoAppKit.ergoValue(
                stakingConfig.stakeTokenId, ErgoValueT.ByteArrayFromHex
            ).getValue(),
            "_stakedTokenID": ErgoAppKit.ergoValue(
                stakingConfig.stakedTokenId, ErgoValueT.ByteArrayFromHex
            ).getValue(),
        ...
        super().__init__(
            ...
            script=os.path.join(
                os.path.dirname(__file__),
                f"ergoscript/{stakingConfig.version}/emission.es",
            ),
            mapping=mapping,
        )
...
class StakingIncentiveContract(PaideiaStakingContract):
    def __init__(self, stakingConfig) -> None:
        mapping = {
            ...
            "_stakePoolKey": ErgoAppKit.ergoValue(
                stakingConfig.stakePoolKey, ErgoValueT.ByteArrayFromHex
            ).getValue(),
            "_dustCollectionReward": ErgoAppKit.ergoValue(
                stakingConfig.dustCollectionReward, ErgoValueT.Long
            ).getValue(),
            "_dustCollectionMinerFee": ErgoAppKit.ergoValue(
                stakingConfig.dustCollectionMinerFee, ErgoValueT.Long
            ).getValue(),
            "_emitReward": ErgoAppKit.ergoValue(
                stakingConfig.emitReward, ErgoValueT.Long
            ).getValue(),
            "_emitMinerFee": ErgoAppKit.ergoValue(
                stakingConfig.emitMinerFee, ErgoValueT.Long
            ).getValue(),
            "_baseCompoundReward": ErgoAppKit.ergoValue(
                stakingConfig.baseCompoundReward, ErgoValueT.Long
            ).getValue(),
            "_baseCompoundMinerFee": ErgoAppKit.ergoValue(
                stakingConfig.baseCompoundMinerFee, ErgoValueT.Long
            ).getValue(),
            "_variableCompoundReward": ErgoAppKit.ergoValue(
                stakingConfig.variableCompoundReward, ErgoValueT.Long
            ).getValue(),
            "_variableCompoundMinerFee": ErgoAppKit.ergoValue(
                stakingConfig.variableCompoundMinerFee, ErgoValueT.Long
            ).getValue(),
        }
        super().__init__(
            stakingConfig,
            script=os.path.join(
                os.path.dirname(__file__),
                f"ergoscript/{stakingConfig.version}/stakingIncentive.es",
            ),
            mapping=mapping,
        )
...
class StakeProxyContract(PaideiaStakingContract):
    def __init__(self, stakingConfig) -> None:
        mapping = {
            ...
            "_stakingIncentiveContract": ErgoAppKit.ergoValue(
                blake2b(
                    bytes.fromhex(
            ...
            "_toStakingIncentive": ErgoAppKit.ergoValue(
                stakingConfig.proxyToStakingIncentive, ErgoValueT.Long
            ).getValue(),
            "_executorReward": ErgoAppKit.ergoValue(
                stakingConfig.proxyExecutorReward, ErgoValueT.Long
            ).getValue(),
            "_minerFee": ErgoAppKit.ergoValue(
                stakingConfig.proxyMinerFee, ErgoValueT.Long
            ).getValue(),
        }
        super().__init__(
            stakingConfig,
            script=os.path.join(
                os.path.dirname(__file__),
                f"ergoscript/{stakingConfig.version}/stakeProxy.es",
            ),
            mapping=mapping,
        )
...
class AddStakeProxyContract(PaideiaStakingContract):
    def __init__(self, stakingConfig) -> None:
        mapping = {
            ...
            "_toStakingIncentive": ErgoAppKit.ergoValue(
                stakingConfig.proxyAddToStakingIncentive, ErgoValueT.Long
            ).getValue(),
            "_executorReward": ErgoAppKit.ergoValue(
                stakingConfig.proxyExecutorReward, ErgoValueT.Long
            ).getValue(),
        ...
        super().__init__(
            ...
            script=os.path.join(
                os.path.dirname(__file__),
                f"ergoscript/{stakingConfig.version}/addStakeProxy.es",
            ),
            mapping=mapping,
        )
...
class UnstakeProxyContract(PaideiaStakingContract):
    def __init__(self, stakingConfig) -> None:
        ...
        super().__init__(
            ...
            script=os.path.join(
                os.path.dirname(__file__),
                f"ergoscript/{stakingConfig.version}/unstakeProxy.es",
            ),
            mapping=mapping,
        )
...
class EmissionBox(ErgoBox):
    def __init__(
        ...
        registers = [
            ErgoAppKit.ergoValue(
                [amountStaked, checkpoint, stakers, emissionAmount],
    ...
    def updateRegisters(self):
        self.registers = [
            ErgoAppKit.ergoValue(
                [self.amountStaked, self.checkpoint, self.stakers, self.emissionAmount],
                ErgoValueT.LongArray,
            )
        ]
...
class StakeBox(ErgoBox):
    def __init__(
        ...
        tokens = {
            self.stakeContract.config.stakeTokenId: 1,
            self.stakeContract.config.stakedTokenId: amountStaked,
        }
        registers = [
            ErgoAppKit.ergoValue([checkpoint, stakeTime], ErgoValueT.LongArray),
    ...
    def updateRegisters(self):
        self.registers = [
            ErgoAppKit.ergoValue(
                [self.checkpoint, self.stakeTime], ErgoValueT.LongArray
            ),
...
class StakeStateBox(ErgoBox):
    def __init__(
        ...
        tokens = {
            self.stakeStateContract.config.stakeStateNFT: 1,
            self.stakeStateContract.config.stakeTokenId: int(1e12) - stakers,
        }
        registers = [
            ErgoAppKit.ergoValue(
                [amountStaked, checkpoint, stakers, checkpointTime, cycleDuration],
                ErgoValueT.LongArray,
            )
        ]

        super().__init__(
            appKit, int(1e6), stakeStateContract.contract, tokens, registers
        )
...
class StakePoolBox(ErgoBox):
    def __init__(
        ...
        tokens = {
            self.stakePoolContract.config.stakePoolNFT: 1,
            self.stakePoolContract.config.stakedTokenId: remaining,
        }
        registers = [
            ErgoAppKit.ergoValue([emissionAmount], ErgoValueT.LongArray),
            ErgoAppKit.ergoValue(
                stakePoolContract.config.stakePoolKey, ErgoValueT.ByteArrayFromHex
            ),
        ]

        super().__init__(
            appKit, int(1e6), stakePoolContract.contract, tokens, registers
        )
    ...
    def updateRegisters(self):
        self.registers = [
            ErgoAppKit.ergoValue([self.emissionAmount], ErgoValueT.LongArray),
...
class StakingIncentiveBox(ErgoBox):
    ...
    @staticmethod
    def fromInputBox(
        inputBox: InputBox, stakingIncentiveContract: StakingIncentiveContract
    ):
...
class StakeProxyBox(ErgoBox):
    def __init__(
        ...
        registers = [
            ErgoAppKit.ergoValue([stakeTime], ErgoValueT.LongArray),
        ...
        super().__init__(
            appKit,
            int(11e6)
            + stakeProxyContract.config.proxyToStakingIncentive
            + stakeProxyContract.config.proxyExecutorReward
            + stakeProxyContract.config.proxyMinerFee,
            stakeProxyContract.contract,
            tokens,
            registers,
        )
    ...
    def updateRegisters(self):
        self.registers = [
            ErgoAppKit.ergoValue([self.stakeTime], ErgoValueT.LongArray),
...
class AddStakeProxyBox(ErgoBox):
    def __init__(
        ...
        tokens = {
            stakeBox.stakeKey: 1,
            self.addStakeProxyContract.config.stakedTokenId: amountToStake,
        }
        registers = [
            ErgoAppKit.ergoValue([int(0)], ErgoValueT.LongArray),
            ErgoAppKit.ergoValue(userErgoTree, ErgoValueT.ByteArrayFromHex),
        ]
        ...
        super().__init__(
            appKit,
            int(1e7)
            + addStakeProxyContract.config.proxyAddToStakingIncentive
            + addStakeProxyContract.config.proxyExecutorReward
            + addStakeProxyContract.config.proxyMinerFee,
            addStakeProxyContract.contract,
            tokens,
            registers,
        )
...
class UnstakeProxyBox(ErgoBox):
    def __init__(
        ...
        registers = [
            ErgoAppKit.ergoValue([int(amountToUnstake)], ErgoValueT.LongArray),
            ErgoAppKit.ergoValue(userErgoTree, ErgoValueT.ByteArrayFromHex),
        ]

        valueAdjustment = (
            int(0) if stakeBox.amountStaked > amountToUnstake else int(1e6)
        )

        super().__init__(
            appKit,
            int(1e7)
            + unstakeProxyContract.config.proxyToStakingIncentive
            + unstakeProxyContract.config.proxyExecutorReward
            + unstakeProxyContract.config.proxyMinerFee
            - valueAdjustment,
            unstakeProxyContract.contract,
            tokens,
            registers,
        )
    ...
    def updateRegisters(self):
        self.registers = [
            ErgoAppKit.ergoValue([self.amountToUnstake], ErgoValueT.LongArray),
            ErgoAppKit.ergoValue(self.userErgoTree, ErgoValueT.ByteArrayFromHex),
        ]
...
class EmitTransaction(ErgoTransaction):
    def __init__(
        ...
        if not stakingConfig.stakeStateContract.validateInputBox(stakeStateInput):
            raise InvalidInputBoxException(
                "Stake state input box does not match contract"
            )
        if not stakingConfig.stakePoolContract.validateInputBox(stakePoolInput):
            raise InvalidInputBoxException(
                "Stake pool input box does not match contract"
            )
        ...
        if not stakingConfig.stakingIncentiveContract.validateInputBox(
            stakingIncentiveInput
        ):
            raise InvalidInputBoxException("Emission input box does not match contract")
        stakeStateBox = StakeStateBox.fromInputBox(
            stakeStateInput, stakingConfig.stakeStateContract
        )
        stakePoolBox = StakePoolBox.fromInputBox(
            stakePoolInput, stakingConfig.stakePoolContract
        )
        emissionBox = EmissionBox.fromInputBox(
            emissionInput, stakingConfig.emissionContract
        )
        stakingIncentiveBox = StakingIncentiveBox.fromInputBox(
            stakingIncentiveInput, stakingConfig.stakingIncentiveContract
        )
        if (
            stakeStateBox.checkpointTime + stakeStateBox.cycleDuration
            > self.preHeader.getTimestamp()
        ):
            raise InvalidTransactionConditionsException("Emission time not reached yet")
        if emissionBox.stakers > 0:
            raise InvalidTransactionConditionsException(
                "Previous emission not finished yet"
            )
        dust = emissionBox.emissionRemaining
        stakePoolBox.remaining = (
            stakePoolBox.remaining - stakePoolBox.emissionAmount + dust
        )
        emissionBox.emissionRemaining = stakePoolBox.emissionAmount - int(
            stakePoolBox.emissionAmount / 100
        )
        ...
        emissionBox.emissionAmount = stakePoolBox.emissionAmount - int(
            stakePoolBox.emissionAmount / 100
        )
        stakeStateBox.amountStaked = (
            stakeStateBox.amountStaked
            + (stakePoolBox.emissionAmount - int(stakePoolBox.emissionAmount / 100))
            - dust
        )
        stakeStateBox.checkpoint = stakeStateBox.checkpoint + 1
        stakeStateBox.checkpointTime = (
            stakeStateBox.checkpointTime + stakeStateBox.cycleDuration
        )
        emissionFeeBox = ErgoBox(
            stakingConfig.appKit,
            int(1e6),
            stakingConfig.appKit.contractFromAddress(
                "9ehtGMAL6gxjjz7TY8QdKnbsXzt8XSgEX4MuMKauf8cZpPumEtU"
            ),
            tokens={
                stakingConfig.stakedTokenId: int(stakePoolBox.emissionAmount / 100)
            },
        )
        stakingIncentiveBox.value -= (
            stakingConfig.emitMinerFee + stakingConfig.emitReward + int(1e6)
        )
        txExecutorBox = ErgoBox(
            stakingConfig.appKit,
            stakingConfig.emitReward,
            stakingConfig.appKit.contractFromAddress(address),
        )
        self.inputs = [
            stakeStateInput,
            stakePoolInput,
        ...
        self.outputs = [
            stakeStateBox.outBox,
            stakePoolBox.outBox,
            emissionBox.outBox,
            emissionFeeBox.outBox,
            stakingIncentiveBox.outBox,
            txExecutorBox.outBox,
        ]
    ...
    def __init__(
        ...
        for stakeInput in stakeInputs:
            if not stakingConfig.stakeContract.validateInputBox(stakeInput):
                raise InvalidInputBoxException(
                    "Stake input box does not match contract"
                )
        emissionBox = EmissionBox.fromInputBox(
            emissionInput, stakingConfig.emissionContract
        )
        ...
        for stakeInput in stakeInputs:
            ...
            if box.checkpoint != emissionBox.checkpoint:
                raise InvalidTransactionConditionsException(
                    "Stake box not on same checkpoint as emission box"
                )
            box.checkpoint = box.checkpoint + 1
            reward = int(
                box.amountStaked * emissionBox.emissionAmount / emissionBox.amountStaked
            )
            stakeRewards += reward
        ...
        reward = int(
            stakingConfig.baseCompoundReward
            + stakingConfig.variableCompoundReward * len(stakeBoxes)
        )
        minerFee = int(
            stakingConfig.baseCompoundMinerFee
            + stakingConfig.variableCompoundMinerFee * len(stakeBoxes)
        )
        ...
        self.outputs = (
            [emissionBox.outBox]
            + stakeBoxes
            + [stakingIncentiveBox.outBox, txExecutorBox.outBox]
        )
        self.fee = minerFee
        self.changeAddress = address
...
class StakeTransaction(ErgoTransaction):
    def __init__(
        ...
        if not stakingConfig.stakeProxyContract.validateInputBox(stakeProxyInput):
            raise InvalidInputBoxException(
                "Stake proxy input box does not match contract"
            )
        stakeProxyBox = StakeProxyBox.fromInputBox(
            stakeProxyInput, stakingConfig.stakeProxyContract
        )
        stakeStateBox = StakeStateBox.fromInputBox(
            stakeStateInput, stakingConfig.stakeStateContract
        )
        ...
        userOutput = stakingConfig.appKit.mintToken(
            ...
            mintAmount=1,
            decimals=0,
            contract=stakingConfig.appKit.contractFromTree(
                stakingConfig.appKit.treeFromBytes(
                    bytes.fromhex(stakeProxyBox.userErgoTree)
                )
            ),
        )
        ...
        stakingIncentiveBox = StakingIncentiveBox(
            stakingConfig.appKit,
            stakingConfig.stakingIncentiveContract,
            stakingConfig.proxyToStakingIncentive,
        )

        txExecutorBox = ErgoBox(
            stakingConfig.appKit,
            stakingConfig.proxyExecutorReward,
            stakingConfig.appKit.contractFromAddress(address),
        )
...
class AddStakeTransaction(ErgoTransaction):
    def __init__(
        ...
        if validations:
            ...
            if not stakingConfig.addStakeProxyContract.validateInputBox(
                addStakeProxyInput
            ):
                raise InvalidInputBoxException(
                    "Add Stake Proxy input box does not match contract"
                )
        stakeBox = StakeBox.fromInputBox(stakeInput, stakingConfig.stakeContract)
        addStakeProxyBox = AddStakeProxyBox.fromInputBox(
            addStakeProxyInput, stakeBox, stakingConfig.addStakeProxyContract
        )
        stakeStateBox = StakeStateBox.fromInputBox(
            stakeStateInput, stakingConfig.stakeStateContract
        )
        ...
        if validations:
            if stakeBox.checkpoint != stakeStateBox.checkpoint:
                raise InvalidTransactionConditionsException(
                    "Stake box is awaiting a compound"
                )
        ...
        userOutput = ErgoBox(
            ...
            contract=stakingConfig.appKit.contractFromTree(
                stakingConfig.appKit.treeFromBytes(
                    bytes.fromhex(addStakeProxyBox.userErgoTree)
                )
            ),
            tokens={stakeBox.stakeKey: 1},
        )
        ...
        stakingIncentiveBox = StakingIncentiveBox(
            stakingConfig.appKit,
            stakingConfig.stakingIncentiveContract,
            stakingConfig.proxyAddToStakingIncentive,
        )
...
class UnstakeTransaction(ErgoTransaction):
    def __init__(
        ...
        if validations:
            ...
            if not stakingConfig.unstakeProxyContract.validateInputBox(
                unstakeProxyInput
            ):
                raise InvalidInputBoxException(
                    "Unstake proxy input box does not match contract"
                )

        stakeBox = StakeBox.fromInputBox(stakeInput, stakingConfig.stakeContract)
        unstakeProxyBox = UnstakeProxyBox.fromInputBox(
            unstakeProxyInput, stakeBox, stakingConfig.unstakeProxyContract
        )
        stakeStateBox = StakeStateBox.fromInputBox(
            stakeStateInput, stakingConfig.stakeStateContract
        )
        ...
        userOutput = ErgoBox(
            ...
            contract=stakingConfig.appKit.contractFromTree(
                stakingConfig.appKit.treeFromBytes(
                    bytes.fromhex(unstakeProxyBox.userErgoTree)
                )
            ),
            tokens=userTokens,
        )
        ...
        stakingIncentiveBox = StakingIncentiveBox(
            stakingConfig.appKit, stakingConfig.stakingIncentiveContract, int(1e8)
        )
...
class CreateStakeProxyTransaction(ErgoTransaction):
    def __init__(
        ...
        stakeProxyBox = StakeProxyBox(
            ...
            amountToStake=amountToStake,
            userErgoTree=stakingConfig.appKit.contractFromAddress(address)
            .getErgoTree()
            .bytesHex(),
            stakeTime=int(time() * 1000),
        )

        if not ErgoAppKit.boxesCovered(
            userInputs, stakeProxyBox.value + int(1e6), stakeProxyBox.tokens
        ):
            raise InvalidTransactionConditionsException(
                "Not enough erg/tokens in the user input boxes"
            )
...
class CreateAddStakeProxyTransaction(ErgoTransaction):
    def __init__(
        ...
        if not ErgoAppKit.boxesCovered(
            userInputs, addStakeProxyBox.value + int(1e6), addStakeProxyBox.tokens
        ):
            raise InvalidTransactionConditionsException(
                "Not enough erg/tokens in the user input boxes"
            )
    ...
    @staticmethod
    def assetsRequired(
        stakingConfig, amountToStake: int, stakeInput: InputBox
    ) -> AssetsRequired:
        stakeBox = StakeBox.fromInputBox(stakeInput, stakingConfig.stakeContract)
        addStakeProxyBox = AddStakeProxyBox(
            appKit=stakingConfig.appKit,
...
class CreateUnstakeProxyTransaction(ErgoTransaction):
    def __init__(
        ...
        if not ErgoAppKit.boxesCovered(
            userInputs, unstakeProxyBox.value + int(1e6), unstakeProxyBox.tokens
        ):
            raise InvalidTransactionConditionsException(
                "Not enough erg/tokens in the user input boxes"
            )
    ...
    @staticmethod
    def assetsRequired(
        stakingConfig, amountToUnstake: int, stakeInput: InputBox
    ) -> AssetsRequired:
        stakeBox = StakeBox.fromInputBox(stakeInput, stakingConfig.stakeContract)
        unstakeProxyBox = UnstakeProxyBox(
            appKit=stakingConfig.appKit,
...
class ConsolidateDustTransaction(ErgoTransaction):
    def __init__(
        self, incentiveDustInputs: List[InputBox], stakingConfig, address: str
    ) -> None:
        super().__init__(stakingConfig.appKit)
        ...
        while not done:
            ...
            for idi in incentiveDustInputs:
                if idi.getValue() < dustCollected:
                    currentDustCollected += (
                        idi.getValue() - stakingConfig.dustCollectionReward
                    )
        ...
        txExecutorBox = ErgoBox(
            stakingConfig.appKit,
            stakingConfig.dustCollectionReward * len(incentiveDustInputs),
            stakingConfig.appKit.contractFromAddress(address),
        )
...
def Ergo_Crux_LPConfig(appKit: ErgoAppKit) -> StakingConfig:
    result = StakingConfig(
        version="1.1",
        appKit=appKit,
        stakeStateNFT="d3c7ce0fc494b4d91ae1925aafc24f3c9202ec55d620e030df3fdae3dcbd3ae9",
        stakePoolNFT="22029bcd60b23e4b0a6926cbfd53a1c6100403df0e6d1b3fac810ed1da51aa5e",
        emissionNFT="521c358ffcfc087b211fac51956c89e3261bf11e6df5119db261b3ec877745bd",
        stakeTokenId="b187a8e347d53f9e42cfd725b27986b06c9078f8f3a9e3f27e92a6f59b4dbfdd",
        stakedTokenId="08be8ef89bb9c2663df6b821ffbc9217ca16c6654020a8b5fe1c74e37c2782fd",
        stakePoolKey="a88124d6ca865334cc3efe7dfb3e1805c2bd3fbc1509e7c15d3d5bfb6691462e",
        stakedTokenName="Ergo_Crux_LP",
        stakedTokenDecimals=0,
        proxyToStakingIncentive=int(1e8),
        proxyAddToStakingIncentive=int(1e7),
        proxyExecutorReward=int(2e6),
        proxyMinerFee=int(2e6),
...
def BootstrapStaking(
    appKit: ErgoAppKit,
    nodeAddress: str,
    tokenId: str,
    stakingStart: int,
    stakingCycleDuration: int,
    dailyEmission: int,
    stakePoolSize: int,
    staked_token_name: str = None,
    version: str = "latest",
) -> StakingConfig:
    res = requests.get(f"{appKit._explorerUrl}/api/v1/tokens/{tokenId}")
    stakedTokenName = staked_token_name if staked_token_name else res.json()["name"]
    stakedTokenDecimals = res.json()["decimals"]
    print(stakedTokenName)
    nodeContract = appKit.contractFromAddress(nodeAddress)
    nodeInputs = appKit.boxesToSpend(
        nodeAddress,
        int(14e6),
        {tokenId: int(stakePoolSize * 10**stakedTokenDecimals)},
    )
    ...
    nodeBox = ErgoBox(
        ...
        int(12e6),
        contract=nodeContract,
        tokens={tokenId: int(stakePoolSize * 10**stakedTokenDecimals)},
    )
    ...
    stakeStateMintTx = appKit.signTransactionWithNode(
        stakeStateMintunsignedTx.unsignedTx
    )
    ...
    stakeTokenMintTx = appKit.signTransactionWithNode(
        stakeTokenMintunsignedTx.unsignedTx
    )
    ...
    stakePoolKeyMintTx = appKit.signTransactionWithNode(
        stakePoolKeyMintunsignedTx.unsignedTx
    )
    ...
    bootstrapUnsignedTx.inputs = [
        stakeStateMintTx.getOutputsToSpend()[0],
        stakePoolMintTx.getOutputsToSpend()[0],
        stakeTokenMintTx.getOutputsToSpend()[0],
        emissionMintTx.getOutputsToSpend()[0],
        stakePoolKeyMintTx.getOutputsToSpend()[1],
    ]
    bootstrapUnsignedTx.outputs = [
        stakeStateBox.outBox,
        stakePoolBox.outBox,
        emissionBox.outBox,
    ]
